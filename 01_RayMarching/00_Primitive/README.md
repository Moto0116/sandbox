...
### 法線とライティング  
  ...
  何してるかわかんないですね。  
  一言でいうとこのコードは距離関数"map(x, y, z)"を偏微分して法線ベクトルを求めてます。(実際は差分法を使用して近似を求めてる)  
  詳しくは説明できないのですが、関数の各成分を偏微分して並べたベクトルを勾配ベクトルと呼び、  
  陰関数の勾配ベクトルは法線として扱うことができます。  
  詳しく知りたい方は「ベクトル解析 勾配」や「ベクトル解析 法線」等で調べていただければ出てくると思います[^5]。  

  [^5]: 今回紹介した差分法を使用して近似を求めるのは結構古いやり方で現在は四面体を使用した方法が主流らしい？
